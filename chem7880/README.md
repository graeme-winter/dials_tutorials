...
The intent is that the instructions allow you to follow along at your own pace. In the general sense the instructions are _not_ beamline specific however as you work through other tutorials you may find that the focus will vary slightly from one tutorial to another, depending on the questions which have been raised. The instructions below are from a different data set to the ones you will have measured last week, so the output will look similar but differ in details around the unit cell and suchlike.

The CLASSE machines seem a little slow on file access, so here I will cut the data set down and only process a subset for speed: if you are doing this in your own time you should use all your data.
...
After connecting to the NX session you need to then use the "CHEM7880 Launcher" to get to more powerful computers to do the processing. It is worth _also_ opening a terminal on the log-in node to allow you to also open the image viewers and look at log files locally.
...
Here select CCP4, more power, 8 CPUs and "go". This puts you on a more powerful computer for doing actual processing. If you open a desktop terminal (not launcher one) you will need to:

```bash
. /nfs/chess/sw/macchess/ccp4_master/ccp4-9/bin/ccp4.setup-sh
```

to be able to "see" the DIALS commands. I have copied all the data from NE-CAT to `/nfs/chess/scratch/user/chem7880/share/NE-CAT` - data for different groups are in:

```console
[gw363@lnx201 gw363]$ ls /nfs/chess/scratch/user/chem7880/share/NE-CAT/images/*
/nfs/chess/scratch/user/chem7880/share/NE-CAT/images/aaron:
grp5

/nfs/chess/scratch/user/chem7880/share/NE-CAT/images/elliot:
grp6

/nfs/chess/scratch/user/chem7880/share/NE-CAT/images/erick:
FAST  Grp4

/nfs/chess/scratch/user/chem7880/share/NE-CAT/images/ilinca:
Grp2  Grp3

/nfs/chess/scratch/user/chem7880/share/NE-CAT/images/jon:
GRP7
```

As you will see it is somewhat chaotic - _next time_ I run this data collection course we will be more systematic!
...
```bash
dials.import ## THE DATA e.g. LIST OF CBF or foo_master.h5 - CBF.bz2 also works
...
so in this case we will want (as an example):

```bash
dials.import /nfs/chess/scratch/user/chem7880/share/NE-CAT/images/ilinca/Grp3/runs/C7886 image_range=1,300
dials.find_spots imported.expt
dials.index imported.expt strong.refl
dials.refine indexed.expt indexed.refl
dials.integrate refined.expt refined.refl
...
There are alternative tracks you can adopt which will be discussed in the longer form narratives below. Also: I have processed one data set using the instructions above so you can also take a look through the output in there while other things are running:

```console
/nfs/chess/scratch/user/chem7880/gw363/example
```
